...
const GridItem = ({
  ...
  const updateGoalDeadline = async () => {
    console.log(supabase.from('goals'))
    try {
      const { data, error } = await supabase
        .from('goals')
        .update({ deadline: calendarDate })
        .eq('id', goalId)
    } catch (error) {
  ...
  const dateClicked = (e, id) => {
    console.log(id);
    setCalendarDate(formatDate(e));
    setGoalId(id);
  }
              ...
              <div className="w-full">
                ...
                <div className="flex w-full items-center justify-between">
                  ...
                  <div className={`duedate ${isChecked && checkedBoxClasses} tracking-wide`}>
                    {`Due: ${calendarDate ?? formatDate(goal.deadline.split('T')[0])}`}
                  </div>
                ...
                {calendarClicked ? <Calendar className={`mt-5`} onChange={(event) => dateClicked(event, goal.id)}/> : null}
              </div>
            )}
          </section>
        ) : (
